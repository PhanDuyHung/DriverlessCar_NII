...
{
	// //get object
	threshold(img,img,60,70,THRESH_BINARY_INV);
	imshow("depththreshold",img);
	...
	//if (contours.size()) imshow("contours",img_contours);
	// vector<Point> approx;
...
void DetectObsThread()
{
	...
	Mat img,roi_img;
	float w = 640,h = 480;
	int roi_width = w/2, roi_height = h * 0.35;
	cv::Rect roi = cv::Rect(w*0.25, h * 0.2,roi_width, roi_height);
    while (1)
    {
...
///////// utilitie functions  ///////////////////////////
bool isStopTraffic = false;
bool isTraffic = false;
bool allowStop = true;
clock_t s;

	//period_1 = 2*CLOCKS_PER_SEC,
	//period_2 = 10*CLOCKS_PER_SEC,
	//period_3 = 5*CLOCKS_PER_SEC;

int labelTrafficStop = -1;
int labelTrafficLeftRight = -1;
bool isReset = false;
...
//tf
	...
	clock_t timeTurnHasTraffic; //20
	clock_t timeTurnHasTrafficEnd;
	bool flagFirstTraffic = true;
	bool flagTurnHasTraffic = true;
	bool flagBarrie = true;
	//bool flag
	//=========== Init  =======================================================
	...
	while (true)
	{
		...
		if (key == 's') {
			...
			running = !running;
			barie = true;
			flagFirstTraffic = true;
			//clear(Q);
			// S = 0;
			// //  Reset=0;
			// tong = 0;
			// int codec = CV_FOURCC('D','I','V', 'X');
  		    // Size output_size(80, 96);
			// bg1.open("bg1.avi", codec, 24, output_size, true);
			// bg2.open("bg2.avi", codec, 24, output_size, true);
			// bg3.open("bg3.avi", codec, 24, output_size, true);

		}
		if (key == 'f') {

			//bg1.release();bg2.release();bg3.release();
		...
		if (running)
		{
			...
			clock_t e = 10*CLOCKS_PER_SEC + s;
			if(clock() > e) allowStop = true;
			...
			// cout <<"time depth"<<time_spent<<endl;
			...
			//NII ne vat can
				...
				// 	setOffset(50);
				// 	hasObs = 0;
				// 	throttle_val = set_throttle_val - 10;
				// }
				// if (timeObs == 0)
				// {
				// 	setOffset(-50);
			...
			if (recordStatus == 'c')
			{
				...
				// cv::Rect roi1 = cv::Rect(200, 150,80, 96);
				// Mat img_roi1 = colorImg(roi1).clone();
				// //bg1.write(colorImg(roi1).clone());

				// cv::Rect roi2 = cv::Rect(280, 150,80, 96);
				// Mat img_roi2 = colorImg(roi2).clone();
				// //bg2.write(colorImg(roi2).clone());

				// cv::Rect roi3 = cv::Rect(360, 150,80, 96);
				// Mat img_roi3 = colorImg(roi3).clone();
				// //bg3.write(colorImg(roi3).clone());

	//NII traffic
				//clock_t begin = clock();

				cv::Rect roired = cv::Rect(280, 150,360,96);
				Mat img_roi_red = colorImg(roired).clone();
				labelTrafficStop = Detecter_NII::GetTrafficSignDetectedRed(img_roi_red);
				if (allowStop && labelTrafficStop == 2) //stop1
				{
					cout <<"===============stop============="<<endl;
				...
				cv::Rect roiblue = cv::Rect(200, 150,240, 96);
				Mat img_roi_blue = colorImg(roiblue).clone();
				labelTrafficLeftRight = Detecter_NII::GetTrafficSignDetectedBlue(img_roi_blue);
				//bg1.write(colorImg(roi1).clone());
				...
				if (flagFirstTraffic || clock() - timeTurnHasTrafficEnd >= 5*CLOCKS_PER_SEC)
				{
					if (labelTrafficLeftRight != -1)
					{
						//hasTraffic = true;
						timeTurnHasTrafficEnd = clock();
						flagFirstTraffic = false;
						//if (set_throttle_val > 45) throttle_val = set_throttle_val - 5;
						if (labelTrafficLeftRight == 1) //turn right2
						{
							cout <<"============================================right================"<<endl;
							// if (isTraffic)
							// {
								if (lane != 1)
								{
									...
									lane = 1;
									clock_t t = clock();
									while(clock() - t <= 12*(CLOCKS_PER_SEC/100)) {
						...
						else if (labelTrafficLeftRight == 0)//turn left3
						{
							cout <<"===============================================left================"<<endl;
							//if (isTraffic)
							//{
